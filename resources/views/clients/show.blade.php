...
<div class="min-h-full flex items-center justify-center sm:px-6 lg:px-8">
    <div class="max-w-4xl w-full space-y-8">
        <h1 class="mt-6 text-center text-3xl font-extrabold text-gray-900">{{ $client->name }}</h1>

        <!-- Client Information -->
        <section class="bg-white shadow-lg rounded-lg p-6 mb-8 border border-gray-200">
            <h2 class="text-3xl font-semibold text-gray-800 mb-4">Información del Cliente</h2>
            <div class="space-y-2 py-2">
                <p><strong class="font-medium">Email:</strong> {{ $client->email }}</p>
                <p><strong class="font-medium">Teléfono:</strong> {{ $client->phone ?? 'No especificado' }}</p>
                <p><strong class="font-medium">Notas:</strong> {{ $client->notes ?? 'Sin notas' }}</p>
            </div>
        </section>
        ...
        <div class="flex justify-between items-center mb-8">
            ...
            <a href="{{ route('clients.sessions.create', $client) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white font-semibold py-2 px-4 rounded-lg transition duration-150 ease-in-out">
                Crear Nueva Sesión
            </a>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white shadow-lg rounded-lg p-6 border border-gray-200">
                ...
                <a href="{{ route('clients.sessions.show', [$client, $session]) }}" class="group relative w-full flex justify-center py-2 px-4 border border-indigo-300 text-sm font-medium rounded-md text-indigo-600 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">Ver detalles</a>
            </div>
            @endforeach
        </div>
        @else
        <p class="text-gray-600">No hay sesiones registradas para este cliente.</p>
        @endif

        <!-- Delete Client Button -->
        <form action="{{ route('clients.destroy', $client) }}" method="POST" class="mt-8">
            @csrf
            @method('DELETE')
            <button type="submit" onclick="return confirm('¿Seguro que quieres eliminar este cliente? Se perderán los datos permanentemente')" class="group relative w-full flex justify-center py-2 px-4 border border-indigo-300 text-sm font-medium rounded-md text-indigo-600 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
